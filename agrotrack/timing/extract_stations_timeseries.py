...
def extract_stations_timeseries(lst_ir, lst_nir, st_info, year):
    ...
    delta_lst_nonan_st = delta_lst_nonan.sel(lat=lat, lon=lon, method="nearest").sel(
        time=year
    )
    ...
    st_data = xr.combine_by_coords(
        [lst_ir_st, lst_nir_st, delta_lst_st, delta_lst_nonan_st]
    )
    return st_data
