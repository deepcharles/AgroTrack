...
def extract_date_from_filename(files):
    if not files:
        ...
        return datetime.strptime(files.split('/')[-1].split('_')[1].split('.')[0],'%Y%j')
    else: 
        return [datetime.strptime(file.split('/')[-1].split('_')[1].split('.')[0],'%Y%j') for file in files]


    # Reading multiple LST files
def create_lst_datacube(years,bounding_box,am_pm = 'am',save_dir='./lst.zarr', to_zarr=False):
    ...
    def preprocess_func(ds):
        ds = ds['LST_Day_1km']
        return clip_around_point(ds,bounding_box).expand_dims(time = [datetime.now()]).chunk({'time':'auto', 'lat': 'auto', 'lon':'auto'})  
